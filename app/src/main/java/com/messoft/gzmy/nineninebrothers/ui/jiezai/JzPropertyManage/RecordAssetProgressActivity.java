...
import com.messoft.gzmy.nineninebrothers.databinding.ActivityRecordAssetProgressBinding;
 ...
 * 判断1：解债师和（会员和高级会员）
 * 判断2：从哪个界面过来的
 * @创建日期 2017/11/9 0009 10:39
 */

public class RecordAssetProgressActivity extends BaseActivity<ActivityRecordAssetProgressBinding>  implements BGASortableNinePhotoLayout.Delegate{

    private PermissionRequest mRequest;
    private String mId;
    private AssetModel mJzModel;
    private AssetProgress mJzProgress; //最上面的进度信息
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_record_asset_progress);
    ...
    private void submit() {
        ...
        DebugUtil.error("RecordAssetProgressActivity", "remarks:" + mStept + "----" + bindingView.bagPhoto.getData().toString());

        doRecord();
    }

    private void doRecord() {
        mJzModel.inputAssetProgress(RecordAssetProgressActivity.this, mId, mStept, mRemarks, new RequestImpl() {
            @Override
            public void loadSuccess(Object object) {
                ...
                if (bindingView.bagPhoto.getData() != null && bindingView.bagPhoto.getData().size() > 0) {
                    ...
                    //提示相应页面刷新
                    RxBus.getInstanceBus().post(new RxBusMessage(RxCodeConstants.REFRESH_ASSET_PROGRESS_LIST, 0));
                    finish();
                }
            }
    ...
    private void uploadImgs(String id, ArrayList<String> data) {
        ...
        HttpUiTips.showDialog(RecordAssetProgressActivity.this, false, "正在上传图片...");
        for (int i = 0; i < data.size(); i++) {
            ...
            final int finalI = i;
            mJzModel.uploadAssetProgressFile(RecordAssetProgressActivity.this,
                    id, str64, ivSuffix, new RequestImpl() {
                        ...
                        @Override
                        public void loadFailed(int errorCode, String errorMessage) {
                            HttpUiTips.dismissDialog(RecordAssetProgressActivity.this);
                            ToastUtil.showToast(errorMessage);
                        }
                    });
        }
    ...
    private void loadInfo() {
        //拿到最新的一个进度（已完成这个进度）
        mJzModel.queryAssetProgressListByAssetId(RecordAssetProgressActivity.this, mId, 0, 1, new RequestImpl() {
            @Override
            public void loadSuccess(Object object) {
                showContentView();
                ...
                if (list.size() > 0) {
                    ...
                    if (mJzProgress.getProgressStage() == 4) {
                        //已完成了，不用录了
                        mStept = 5;
                        ToastUtil.showToast("进度已完成，无须录制");
                        finish();
                    } else {
                        mStept = mJzProgress.getProgressStage() + 1;
                        bindingView.tvStage.setText(BusinessUtils.assetProgressStage(mStept + ""));
                    }
                } else {
                    //没查到就从1阶段录起
                    mStept = 1;
...
//                ToastUtil.showToast("申请权限成功");
                ...
                startActivityForResult(BGAPhotoPickerActivity.newIntent(RecordAssetProgressActivity.this, takePhotoDir,
            ...
            public void onFailure(List<String> permissions) {
                ...
                if (AndPermission.hasAlwaysDeniedPermission(RecordAssetProgressActivity.this, permissions)) {
                    // 第一种：用默认的提示语。
                    AndPermission.defaultSettingDialog(RecordAssetProgressActivity.this, 110).show();
                }
            }
        });
